...
target_direct_play = '''  const isDirectPlay = useMemo(() => {
...
replacement_direct_play = '''  const isDirectPlay = useMemo(() => {
    const ext = (media.filePath || '').toLowerCase();
    const isNativeContainer = ext.endsWith('.mp4') || ext.endsWith('.m4v') || ext.endsWith('.webm');
    if (!isNativeContainer || selectedQuality !== 'original') return false;
...
client_code = client_code.replace(target_direct_play, replacement_direct_play)

target_stream_mode = '''  // Stream mode status for user transparency
...
replacement_stream_mode = '''  // Stream mode status for user transparency
  const streamMode = useMemo(() => {
    const selectedTrackObj = audioTracks.find(t => t.streamIndex === selectedAudioTrack) || audioTracks[0];
    const rawAudioCodec = (selectedTrackObj?.codec || media.audioCodec || 'aac').toLowerCase();
    const rawVideoCodec = (media.videoCodec || 'h264').toLowerCase();
...
client_code = client_code.replace(target_stream_mode, replacement_stream_mode)
...
target_server_codecs = '''    // Check if video can be directly stream-copied without re-encoding (Lossless Direct Stream Copy)
...
replacement_server_codecs = '''    // Check if video can be directly stream-copied without re-encoding (Lossless Direct Stream Copy)
    const supportedVideoCodecs = ['h264', 'hevc', 'h265', 'vp8', 'vp9', 'av1'];
...
server_code = server_code.replace(target_server_codecs, replacement_server_codecs)

target_server_fmp4 = '''    const isHevc = media.videoCodec === 'hevc' || media.videoCodec === 'h265';
...
replacement_server_fmp4 = '''    const isHevc = media.videoCodec === 'hevc' || media.videoCodec === 'h265';
...
server_code = server_code.replace(target_server_fmp4, replacement_server_fmp4)
